export default {
  title: {
    home: 'Utama',
    login: 'Log masuk',
    lists: { 0: 'Senarai' },
    tags: {
      0: 'Penanda',
      1: '{amount} penanda untuk kategori {name}',
      2: 'Cari tag'
    },
    ideas: {
      0: 'Inspirasi',
      1: '«{amount}»'
    },
    sounds: { 0: 'Bunyi' },
    leaderboard: { 0: 'Papan pendahulu' },
    extend: { 0: 'Lebih | Kurang' },
    skin: {
      0: 'Skin ini mempunyai elemen-elemen daripada skin-skin yang lain',
      1: 'Klik di sini untuk maklumat lanjut'
    },
    tools: 'Alat'
  },
  home: {
    0: 'Tersedia untuk muat turun',
    1: 'Kebanyakkan: _name_ skins',
    2: 'Ditonton | Disukai | Dimuat turun',
    3: 'Terbitkan skin baharu',
    4: 'Cara untuk memuat naik skin',
    5: '{amount} Skin | {amount} Skin | {amount} Skin'
  },
  changelogs: {
    0: 'Log perubahan',
    1: 'Senarai semua penyemakan yang telah dibuat di dalam versi terkini',
    2: 'Tiada apa-apa di sini',
    3: 'Tetapi anda boleh menambah perubahan yang anda telah buat kepada skin versi ini',
    4: 'Tidak dikumpulkan',
    5: 'Baharu',
    6: 'Pembetulan',
    7: 'Perubahan',
    8: 'Pembuangan'
  },
  ideas: {
    0: 'Tiada apa-apa dijumpai',
    1: 'Screenshot untuk «{type}» tidak dimuat naik kepada site',
    2: '«{amount}» {name}'
  },
  creators: {
    0: 'Pembuat',
    1: 'Senarai semua pembuat yang terlibat dalam pembuatan versi ini',
    2: 'Tiada sesiapa di sini',
    3: 'Mencari untuk nama pemain akan menyenaraikan profil-profil.',
    4: 'Penyunting',
    5: 'Peranan',
    6: 'Versi-versi'
  },
  tags: {
    0: 'Penanda',
    1: 'Senarai semua penanda yang berkait dengan skin ini',
    2: 'Ia kelihatan sunyi di sini.',
    3: 'Gunakan medan di atas untuk mencari penanda-penanda yang baharu',
    4: 'Tag-tag tidak dijumpai',
    5: 'Cari perkara lain',
    6: 'Cari'
  },
  videos: {
    0: 'Video-video',
    1: 'Senarai semua video penunjukkan untuk skin ini',
    2: 'Tiada video penunjukkan',
    3: 'Layari untuk beberapa di YouTube atau buat sendiri anda punya video penunjukkan'
  },
  sources: {
    0: 'Atribusi',
    1: 'Senarai semua aset atau peralatan yang digunakan untuk versi skin ini',
    2: 'Nampak macam kosong kat sini',
    3: 'Jika anda menggunakan aset daripada skin lain, atau seni lain, sila mengisinya di dalam kotak di atas',
    4: 'Tidak dikumpulkan'
  },
  checks: {
    0: 'Semua',
    1: 'Metadata',
    2: 'Sorotan',
    3: 'Nama skin tidak dispesifikasi',
    4: 'Menambah sekurang-kurangnya satu pembuat skin',
    5: 'Letakkan beberapa tag',
    6: 'Saiz skin tidak dispesifikasi',
    // 7: 'Select at least one gamemode',
    8: 'Pilih resolusi gambar (SD atau HD)',
    9: 'Pilih sekurang-kurangnya satu nisbah aspek',
    10: 'Tiada screenshot yang diberikan',
    11: 'Masukkan sekurang-kurangnya satu screenshot bagi gameplay',
    12: 'Pautan bagi skin tidak dispesifikasi',
    20: 'Nampak cun',
    21: 'Semua issue dibaiki. Tahniah!',
    22: 'Senarai semak',
    23: 'Letakkan sekurang-kurangnya satu versi awam',
    24: 'Letakkan sekurang-kurangnya satu pengarang kepada versi ini',
    25: 'Sebahagian pereka tiada peranan',
    26: '{amount} screenshot tidak mempunyai kategori',
    28: 'Pilih gamemode untuk tangkapan skrin permainan'
  },
  skins: {
    0: 'Skin-skin',
    1: 'Popular',
    2: 'std | ctb',
    3: 'mania | taiko',
    4: 'Terbaharu',
    5: 'Rawak',
    6: 'Semua skin',
    7: 'Nisbah aspek',
    8: 'Semua skin | skin osu',
    9: 'Skin | Skin | Skin',
    10: 'Skin osu oleh {name} | Semua skin daripada',
    11: 'Cari',
    12: '{name} skin-skin',
    13: 'Meneroka skin lain',
    14: 'Skin-skin popular'
  },
  skin: {
    0: 'Mengedit',
    1: 'Tambahan',
    2: 'HD',
    3: 'FullHD',
    4: 'Aminasi',
    5: 'Screenshots',
    6: 'Gameplay sahaja',
    7: 'Pautan muat turun | Muat turun skin',
    8: 'Pos forum | Forum osu!',
    9: 'Pos Reddit',
    10: 'Pembuat',
    11: 'Penanda',
    13: 'Video-video',
    14: 'Versi baharu | Mengurus versi',
    15: 'Mengemaskini | Menyerahkan',
    16: 'Mod permainan',
    17: 'Nama skin',
    18: 'Versi | Versi-versi',
    19: 'Saiz',
    20: 'Metadata',
    21: 'Penilaian',
    22: 'Muat turun',
    23: 'Muat turun akan disediakan nanti',
    24: '{amount} saat tinggal',
    25: 'Kamu bukan salah satu dari kami',
    26: 'Kelihatan seperti kamu bukan pembuat atau penyunting skin ini',
    27: 'Skin oleh',
    28: 'Tiada tag',
    29: 'Tiada video',
    30: 'Tiada pembuat',
    31: 'Nilaikan skin ini',
    32: 'Undi naik & komen di reddit',
    33: 'Komen di osu! forum',
    34: 'Skin lain daripada pengarang ini | Skin lain daripada pengarang ini',
    35: 'Sokong skin ini',
    36: 'Tidak disediakan',
    37: 'Malangnya, skin ini telah dibuang',
    38: 'Selamat datang ke penyunting pos skin!',
    39: `Baru kepada penyunting skin? Biar kami memberi panduan melalui antara muka dan tunjuk kepada anda potensi penuh penyunting ini!`,
    40: '{current} daripada {max} selesai',
    41: 'Lihat semula kemudian',
    42: `Mari kita mula`,
    43: 'Tahniah!',
    44: 'Anda dengan berjayanya telah siap tutorial ini, silakan!',
    45: 'pemuat naik'
  },
  drag: {
    0: 'Ketik di sini',
    1: 'atau tarik dan lepas gambar di sini'
  },
  footer: {
    0: 'Dibuat oleh cyperdark',
    1: 'Penghargaan',
    2: 'Peraturan',
    3: 'Kenalan'
  },
  credits: {
    0: 'Chibi',
    1: 'Lukisan',
    2: 'Ikon-ikon',
    3: 'Semua ikon diambil daripada {name}',
    4: 'Bendera-bendera',
    5: 'Semua gambar bendera diambil dari {name}',
    6: 'Lokalisasi',
    7: 'Sesiapa? [{lang}]',
    8: 'Nampak macam cpol lupa nak tambah. {text}'
  },
  search: {
    0: 'Cari skin',
    1: 'Tontonan',
    2: 'Tajuk',
    3: 'Penilaian',
    4: 'Saiz',
    5: 'Tarikh',
    7: 'Tiada apa-apa dijumpai',
    8: 'Cuba gunakan penapis carian yang lain',
    9: 'Menunggu pertanyaan',
    10: 'Gunakan penapis carian atau taip tajuk skin, pembuat atau penanda',
    11: 'Muat turun',
    12: 'Nama',
    13: {
      0: 'Skin-skin',
      1: 'Penanda',
      2: 'Pengguna',
      3: 'Senarai'
    },
    14: 'Carian:',
    15: 'Saiz:',
    16: 'Penilaian:',
    17: 'Tontonan:',
    18: 'Muat turun:'
  },
  contests: {
    0: 'Pertandingan-pertandingan',
    1: 'Siaran berita rasmi',
    2: 'Penyerahan',
    3: 'Sedang menyediakan',
    4: 'Undian',
    5: 'Telah siap',
    6: 'Pertandingan-pertandingan',
    7: 'Laman mengundi'
  },
  rules: {
    0: 'Kemas kini terakhir pada',
    1: 'Garis-garis panduan mengepos skin'
  },
  skinini: {
    0: 'Selamat datang ke penyunting skin.ini!',
    1: `Baru kepada penyunting skin.ini? Biar kami memberi panduan melalui antara muka dan tunjuk kepada anda potensi penuh penyunting ini!`,
    2: '{current} daripada {max} selesai',
    3: 'Tiada hasil',
    4: 'Pertanyaan anda kemungkinan ada typo'
  },
  tabs: {
    0: 'Cari',
    1: 'Mengedit',
    2: 'Senarai'
  },
  undefined: {
    0: 'Kosong',
    1: 'Malangnya tak ada apa-apa kat sini, tapi ko boleh tengok halaman lain',
    2: 'Malangnya tak ada apa-apa kat sini',
    3: 'Tiada kebenaran',
    4: 'Anda perlu log masuk untuk mengedit skin ini.',
    5: 'Anda perlu log masuk untuk menambah skin baharu.'
  },
  settings: {
    0: 'Aturcara',
    1: 'Skroi tetikus',
    2: 'Membolehkan leret tuding tetikus untuk karusel imej, membenarkan navigasi kiri-ke-kanan atau kanan-ke-kiri.',
    3: 'Zoom masuk',
    4: 'Membolehkan zoom imej berlegar atas untuk membolehkan pratonton skin.'
  },
  meta: {
    title: {
      0: 'Meneroka skin-skin osu',
      1: 'Mod permainan, nisbah aspek skin osu',
      2: 'Skin osu yang popular',
      3: 'Mencari skin osu',
      4: 'Senarai penanda, {name}',
      5: '{name} v{version} skin osu',
      6: 'Penghargaan',
      7: 'skin-skin osu',
      8: '{name} mencari skin osu',
      9: 'Dapatkan inspirasi dari skin osu yang lain',
      10: '{name} membuat skin osu',
      11: 'Cari tag',
      12: '{name} tag-tag dijumpai',
      13: 'Skin baharu',
      14: 'Peraturan laman web',
      15: 'Pertandingan-pertandingan osu!',
      16: 'peralatan mengkin osu!'
    },
    description: {
      0: 'Dapatkan skin osu! yang terbaik dari laman web kami! Kami mempunyai banyak skin untuk semua mod permainan, nisbah aspek dan juga pemain-pemain terbaik. Tambahan, kami juga mempunyai bahagian yang istimewa untuk peminat-peminat anime. Menaik taraf permainan anda dengan skin yang sempurna. #osuskins #osugame #anime',
      1: '{views} tontonan. osu skin untuk {modes}, saiznya adalah {size}, dibuat oleh {creators}. Menyokong {ratios}',
      2: 'Cari skin osu! yang sempurna dengan ciri carian kami yang hebat. Semak melalui koleksi kami yang luas, ditapis dengan mod permainan, aspek nisbah, dan banyak lagi. Atau, lihat skin yang digunakan oleh pemain-pemain terbaik.',
      3: 'Meneroka skin yang pelbagai untuk mod permainan dan aspek nisbah di laman web kami. Pilih daripada mod standard, catch the beat, mania, dan taiko. Cari skin yang sempurna untuk pemainan anda.',
      4: 'Cari osu skin yang sempurna daripada permain terkenama di website kami. Imbas kembali koleksi skin terkemuka kami dan hiasi gameplay anda. Ketemuilah pelbagai gaya dan tema yang menepati pilihan anda.',
      5: 'Senarai tag yang wujud untuk skin osu.',
      6: '{name} koleksi skin osu!, muat turun secara percuma',
      7: 'Skin osu yang banyak untuk {name}, muat turun secara percuma!',
      8: 'Semua skin osu dibuat oleh {name}',
      9: 'Cari inspirasi untuk skin osu! anda yang seterusnya di laman web kami. Semak melalui koleksi skin yang pelbagai dan teroka pelbagai teknik dan idea. Dapatkan inspirasi dan cipta skin anda yang unik.',
      10: 'Cari tag-tag dari kesemua tag di site ini.',
      11: 'Senarai semua pertandingan-pertandingan osu! yang aktif/selesai',
      12: '{amount} entri untuk pertandingan {title}. {views} tontonan'
    }
  },
  buttons: {
    0: 'Daftar masuk',
    1: 'Muka depan',
    profile_osu: 'profil osu',
    new_skin: 'Skin baharu',
    notifications: 'Notifikasi',
    settings: 'Aturcara',
    logout: 'Keluar',
    load_more: 'Tambah lagi',
    copy_short: 'Menyalin pautan pendek',
    copied: 'Telah disalin',
    reddit_link: 'Reddit',
    osu_forum_link: 'forum osu',
    skin_return: 'Balik ke skin',
    source: 'Sumber',
    contest_voting: 'Undi disini',
    rules_agree: 'Saya setuju',
    report_issue: 'Laporkan masalah',
    ghost_creator: 'Pencipta hantu',
    close: 'Tutup',
    quick_select: 'Pilihan pantas'
  },
  choose: {
    0: 'Pilihan',
    1: 'Diperlukan',
    2: 'Pilih sekurang-kurangnya satu pilihan'
  },
  user: {
    roles: {
      0: 'Pencipta',
      1: 'Penggubah',
      2: 'Animasi',
      3: 'Lukisan',
      4: 'Kolaborasi',
      5: 'Bunyi',
      6: 'Gameplay'
    }
  },
  tooltips: {
    0: 'Tontonan: {amount}',
    1: 'Muat semula hasil',
    2: 'Menurun | Menaik',
    3: 'Suka: {likes} / Tidak suka: {dislikes}',
    4: 'Tekan ctrl+tag untuk menambah tag',
    5: 'Edit versi',
    6: '{s}Dafter masuk{p}, untuk menilai skin ini',
    7: 'Tiada kandungan',
    8: 'Tiada url',
    9: 'Muat turun: {amount}',
    10: 'Set semua semuanya',
    11: 'Tunjuk panduan',
    12: 'Tutup panduan',
    13: 'Untuk mendandakan suatu peraturan sudah dibaca, letakkan kursor diatasnya (Kuning menunjukkan belum dibaca)',
    14: {
      0: '[hl]{example}[/hl] - [b]equal[/b] to {amount}',
      1: '[hl]{example}[/hl] - [b]above[/b] {amount}',
      2: '[hl]{example}[/hl] - [b]below[/b] {amount}',
      3: '[hl]{example}[/hl] - [b]from[/b] {amount} [b]to[/b] {amount_2}'
    },
    15: 'Nyahpilih',
    16: 'Buang file'
  },
  lists: {
    screens: {
      0: 'Selamat datang',
      1: 'Menu utama',
      2: 'Pilih lagu',
      3: 'Menu mode',
      4: 'Mod',
      5: 'Kiraan',
      6: 'Gameplay',
      7: 'Spinner',
      8: 'Section pass',
      9: 'Section Fail',
      10: 'Skrin berhenti',
      11: 'Skrin Gagal',
      12: 'Skrin keputusan',
      13: 'Skip',
      14: 'Banner',
      15: 'Skrin keputusan (dibawah)',
      16: 'Permulaan imej'
    },
    cats: {
      0: 'NSFW',
      1: 'Furry',
      2: 'Digubah'
    }
  },
  select: {
    0: 'Baharu',
    1: 'Telah dibetulkan',
    2: 'Telah ditukar',
    3: 'Telah dibuang',
    4: 'Lukisan',
    5: 'Font',
    6: 'Pelukis',
    7: 'Unsur skin',
    8: 'Audio/Sfx',
    9: 'Alat',
    10: 'Imej',
    views: 'Tontonan',
    likes: 'Suka',
    dislikes: 'Tidak suka',
    downloads: 'Muat turun',
    ratio: 'Pilih nisbah aspek | nisbah',
    sorting: 'Desc. | Asc.'
  },
  sorting: {
    0: 'Paling | Kurang',
    1: 'Paling | Kurang',
    2: 'Paling Tinggi | Paling Rendah',
    3: 'Paling Besar | Paling Kecil',
    4: 'Terbaharu | Paling Lama',
    5: 'A-Z | Z-A'
  },
  placeholders: {
    0: 'Pilih peranan',
    1: 'Pilih versi',
    2: 'Taip sedikit teks...',
    3: 'Pilih julat saiz',
    4: 'Pilih julat tarikh',
    5: 'Tahun',
    6: 'Bulan',
    7: 'Pilih nisbah aspek',
    8: 'Pilih kategori',
    9: 'Mod permainan',
    10: 'Pilih taip',
    11: 'Letakkan pautan Youtube',
    12: 'Taip penerangan dan url. Format penerangan:url',
    13: 'Pilih tambahan',
    14: 'Letakkan pautan',
    15: 'Tajuk skin, pengarang atau tag',
    16: 'Pilih katergori',
    17: 'Tulis nama penanda',
    18: 'Taip nama skin',
    19: 'Taip nama pereka ',
    20: 'Pilih fail',
    24: 'Tulis nama permainan...',
    25: 'Tulis julat',
    26: 'Tulis teks'
  },
  contribute: { 0: 'Sumbangan' },
  nsfw: {
    0: 'Dilarang, lucah!',
    1: 'Skin ini mengandungi kandungan sensitif ataupun kandungan dewasa',
    2: 'Skin ini mengandungi kandungan sensitif atau kandungan dewasa yang mungkin tidak disukai oleh sesetengah pengguna',
    3: 'Ok',
    4: 'jumpa lagi o/',
    5: 'Amaran Furry!'
  },
  removed: {
    0: 'Versi tidak ada',
    1: 'Pengarang skin telah melupuskan versi ini daripada site.',
    2: 'Tukar kepada versi yang ada'
  },
  notify: {
    0: 'Masalah tidak dapat dibaiki',
    1: 'Tidak kesemua bahagian telah diisi',
    2: 'Tunjuk',
    3: 'Tidak dijumpai',
    4: 'Banner tidak dijumpai (Screenshot pertama sedang digunakan sebagai banner)',
    5: 'Tiada perubahan',
    6: 'Tiada perubahan dijumpai',
    7: 'ID tidak dijumpai',
    8: 'ID untuk video tidak dijumpai, cuba pautan yang lain (Buat masa sekarang hanya pautan Youtube sahaja yang disokong)',
    9: 'Alamak',
    10: 'Kelihatan seperti sesi anda telah tamat tempoh, buka helaian baharu dan log masuk akaun anda (Takpe, semuanya telah disimpan secara automatik)',
    11: 'Wujud',
    12: 'Video ini telah pun ditambah',
    13: 'Wujud',
    14: 'Tag ini telah pun ditambah',
    15: 'Tidak dijumpai',
    16: 'Anda tidak boleh memadamkan tag',
    17: 'Telah ditambah',
    18: 'Anda berjaya menambah tag baharu',
    19: 'Alamak',
    20: 'Pencipta ini telah pun diletakkan',
    21: 'EZ',
    22: 'Pencipta baharu telah pun ditambah',
    23: 'HUH',
    24: 'Mustahil untuk memadam sesebuah pencipta',
    25: 'Salinan',
    26: 'Penukaran ini telah ditambah sebentar tadi.',
    27: 'Salinan',
    28: 'Url ini telah ditambah sebentar tadi.',
    29: 'Amaran',
    30: 'Anda lupa untuk menambah penerangan untuk url. Contohnya: Penerangan:url',
    31: 'Amaran',
    32: 'Anda lupa untuk menambah url untuk penerangan. Contohnya: Penerangan:url',
    33: 'Telah dipulihkan',
    34: 'Versi {name} telah dipulihkan',
    35: 'Dibuang',
    36: 'Versi {name} telah dibuang',
    37: 'Ralat',
    38: 'Nampaknya seperti server mensyaki anda ialah bot.',
    39: 'Sesi log masuk semasa anda telah tamat. Untuk meneruskan, sila buka laman baru dan log semula ke dalam akaun anda. Setelah berbuat demikian, sila kembali semula ke laman ini dan cuba semula.',
    40: 'Maklumat kendiri tidak betul (log masuk semula ke dalam akaun. Takpe, semuanya telah disimpan secara automatik)',
    41: 'Ralat tidak diketahui',
    42: 'Anda tidak boleh mengedit skin yang tidak wujud',
    43: 'Nampaknya anda tidak mempunyai kebenaran untuk mengedit skin ini',
    44: 'Anda tidak boleh menyerahkan versi ini kerana ianya sama seperti yang sebelumnya.',
    45: 'Semua penukaran telah dihantar untuk semakan',
    46: 'Berjaya',
    47: 'Perubahan baharu telah diserahkan untuk semakan',
    48: 'Perubahan telah dimuat naik kepada pihak moderasi',
    49: 'Link pelik',
    50: 'Link ini tidak boleh diguna sebagai link muat turun',
    51: 'Telah diserahkan untuk semakan',
    52: 'Amaran',
    53: 'Sila pilih jenis aset',
    54: 'Skin ini sedang dibawah moderasi. Sila bersabar sementara ia sedang diproses',
    55: 'Berjaya',
    56: 'Pencipta hantu baharu telah pun ditambah',
    57: 'Warna disalin',
    58: `Kod RGB "{color}" sudah tersedia di papan klip anda!`
  },
  header: {
    0: 'Notifikasi',
    1: 'Ikuti kami di platform-platform berikut untuk menerima pemberitahuan berkenaan skin-skin baharu'
  },
  restricted_view: {
    0: 'Tiada kebenaran',
    1: 'Sila log masuk dengan akaun anda',
    2: 'Pengelihatan terhad',
    3: 'Helaian ini belum dikeluarkan, lagi.'
  },
  confirm: {
    0: 'Set semula segalanya?',
    1: 'Tunjuk panduan?',
    2: 'Hantar skin?',
    3: 'Ya',
    4: 'Tidak',
    5: 'Set semula .ini?'
  }
};